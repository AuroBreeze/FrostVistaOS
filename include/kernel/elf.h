...
struct elfhdr {
	...
	uint32 version;
	uint64 entry; // gives the virtual address to which the system first
		      // transfers control, thus starting the process
	uint64 phoff; // holds the program header table's file offset in bytes
	uint64 shoff; // Offset from the beginning of the file to the section
		      // header
...
struct proghdr {
	uint32 type;
	uint32 flags;
	uint64 off; // gives the offset from the beginning of the file at which
		    // the first byte of the segment resides
	uint64 vaddr; // gives the virtual address at which the first byte of
		      // the segment resides in memory
	uint64
	    paddr; // On systems for which physical addressing is relevant, this
		   // member is reserved for the segment's physical address
	uint64 filesz; // gives the number of bytes in the file image of the
		       // segment
	uint64 memsz; // gives the number of bytes in the memory image of the
		      // segment
	uint64 align;
};
