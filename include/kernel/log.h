...
#define LOG_TRACE(fmt, ...)                                                    \
	do {                                                                   \
		if (CURRENT_LOG_LEVEL <= LOG_LEVEL_TRACE)                      \
			kprintf(LOG_TS_COLOR                                   \
				"%s" LOG_COLOR_RESET LOG_TAG_TRACE             \
				"[TRACE] " LOG_MODULE_TAG fmt LOG_COLOR_RESET  \
				"\n",                                          \
				log_ts(), ##__VA_ARGS__);                      \
	} while (0)
...
#define LOG_INFO(fmt, ...)                                                     \
	do {                                                                   \
		if (CURRENT_LOG_LEVEL <= LOG_LEVEL_INFO)                       \
			kprintf(LOG_TS_COLOR                                   \
				"%s" LOG_COLOR_RESET LOG_TAG_INFO              \
				"[ INFO] " LOG_MODULE_TAG fmt LOG_COLOR_RESET  \
				"\n",                                          \
				log_ts(), ##__VA_ARGS__);                      \
	} while (0)
...
#define LOG_DEBUG(fmt, ...)                                                    \
	do {                                                                   \
		if (CURRENT_LOG_LEVEL <= LOG_LEVEL_DEBUG)                      \
			kprintf(LOG_TS_COLOR                                   \
				"%s" LOG_COLOR_RESET LOG_TAG_DEBUG             \
				"[DEBUG] " LOG_MODULE_TAG fmt LOG_COLOR_RESET  \
				"\n",                                          \
				log_ts(), ##__VA_ARGS__);                      \
	} while (0)
...
#define LOG_WARN(fmt, ...)                                                     \
	do {                                                                   \
		if (CURRENT_LOG_LEVEL <= LOG_LEVEL_WARN)                       \
			kprintf(LOG_TS_COLOR                                   \
				"%s" LOG_COLOR_RESET LOG_TAG_WARN              \
				"[ WARN] " LOG_MODULE_TAG fmt LOG_COLOR_RESET  \
				"\n",                                          \
				log_ts(), ##__VA_ARGS__);                      \
	} while (0)
...
#define LOG_ERROR(fmt, ...)                                                    \
	do {                                                                   \
		if (CURRENT_LOG_LEVEL <= LOG_LEVEL_ERROR)                      \
			kprintf(LOG_TS_COLOR                                   \
				"%s" LOG_COLOR_RESET LOG_TAG_ERROR             \
				"[ERROR] %s:%d: " LOG_MODULE_TAG               \
				    fmt LOG_COLOR_RESET "\n",                  \
				log_ts(), __FILE__, __LINE__, ##__VA_ARGS__);  \
	} while (0)
