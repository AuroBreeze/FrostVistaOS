...
int main(int argc, char *argv[]) {
  ...
  root_inode.blocks[0] = super_block.data_area_start; // Data block for Root
  ...
  // Allocate data blocks for init binary sequentially after the root block
  ...
  fseek(img,
        super_block.inode_area_start * BLOCK_SIZE + sizeof(struct disk_inode),
        SEEK_SET);
  ...
  // Update Inode Bitmap (Inode 0 and 1 are used -> 00000011 in binary -> 0x03)
  ...
  // For example, if init needs 2 blocks, total used is 3 blocks -> 00000111 ->
  // 0x07
