...
void test_virtio_disk() {
  ...
  // We want to read the first block (Block 0)
  test_buf.blkno = 0;
  test_buf.done = 0;

  LOG_DEBUG("--- VirtIO Disk Test: Start READ ---");

  // 2. Call your newly baked rw function (0 means READ)
  // This will block the current thread until the PLIC interrupt wakes it up
  virtio_disk_rw(&test_buf, 0);

  LOG_DEBUG("--- VirtIO Disk Test: READ Success! ---");

  // 3. Print the first 16 bytes to verify
  // Since disk.img is created from /dev/zero, they should all be 0x00
  for (int i = 0; i < 16; i++) {
    LOG_DEBUG("Block 0, Byte %d: 0x%02x", i, test_buf.data[i]);
  }

  // 4. Now let's test WRITE!
  // We write a magic signature to the first two bytes
  test_buf.data[0] = 0xAA;
  test_buf.data[1] = 0xBB;
  test_buf.done = 0; // Reset the done flag before sending again!

  LOG_DEBUG("--- VirtIO Disk Test: Start WRITE ---");

  // Call rw function (1 means WRITE)
  virtio_disk_rw(&test_buf, 1);

  LOG_DEBUG("--- VirtIO Disk Test: WRITE Success! ---");
}
