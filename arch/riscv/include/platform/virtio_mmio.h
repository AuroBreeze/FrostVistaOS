...
#define VIRTIO_MMIO_VIRT_BASE (0x10001000 + KERNEL_VIRT_OFFSET)
...
#define VIRTIO_CONFIG_S_ACKNOWLEDGE                                            \
  1 // Indicates that the guest OS has found the device and recognized it as a
    // valid virtio device.
#define VIRTIO_CONFIG_S_DRIVER                                                 \
  2 // Indicates that the guest OS knows how to drive the device. Note: There
...
#define VIRTIO_CONFIG_S_FAILED                                                 \
  128 // Indicates that something went wrong in the guest, and it has given up
...
#define VIRTIO_CONFIG_S_DEVICE_NEEDS_RESET                                     \
  64 // Indicates that the device has experienced an error from which it can’t
     // recover.
...
#define VIRTIO_DEVICE_FEATURES                                                 \
  0x010 // will return bits DeviceFeaturesSel ∗ 32 to (DeviceFeaturesSel ∗ 32) +
        // 31, eg. feature bits 0 to 31 if DeviceFeaturesSel is set to 0 and
...
#define VIRTIO_QUEUE_NUM_MAX                                                   \
  0x034 // Reading from the register returns the maximum size (number of
        // elements) of the queue the device is ready to process or zero (0x0)
        // if the queue is not available. This applies to the queue selected by
        // writing to QueueSel.
#define VIRTIO_QUEUE_NUM                                                       \
  0x038 // W Queue size is the number of elements in the queue. Writing to this
        // register notifies the device what size of the queue the driver will
        // use.

#define VIRTIO_QUEUE_READY                                                     \
  0x044 // RW Writing one (0x1) to this register notifies the device that it can
...
#define VIRTIO_INTERRUPT_ACK                                                   \
  0x064 // W Writing a value with bits set as defined in InterruptStatus to this
...
#define VIRTIO_BLK_F_CONFIG_WCE                                                \
  11 // Device can toggle its cache between writeback and writethrough modes.
#define VIRTIO_BLK_F_DISCARD                                                   \
  13 // Device can support discard command, maximum discard sectors size in
     // max_discard_sectors and maximum discard segment number in
     // max_discard_seg.
#define VIRTIO_BLK_F_WRITE_ZEROES                                              \
  14 // Device can support write zeroes command, maximum write zeroes sectors
     // size in max_write_zeroes_sectors and maximum write zeroes segment number
     // in max_write_zeroes_seg.
...
struct virtq_avail {
  uint16 flags;
  /* idx field indicates where the driver would put the next descriptor entry in
   * the ring (modulo the queue size). */
  uint16 idx;
...
struct virtq_used {
   ...
   * beyond the first len bytes, and SHOULD ignore this data. */
